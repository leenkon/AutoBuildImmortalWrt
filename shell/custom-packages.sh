...
# 同样是代理相关 但以下 2 个属于 imm 仓库内的软件 一般在 build24.sh 中已经集成 你也可以在此处调整它的去留 若去除组件则使用减号 - 若添加则 不使用减号 或者 不处理
...
CUSTOM_PACKAGES="$CUSTOM_PACKAGES ddns-scripts"
CUSTOM_PACKAGES="$CUSTOM_PACKAGES ddns-scripts-services"
